...
public class ServerDemo {
	public static void main(String[] args) throws IOException {
		...
		SocketAddress target = new InetSocketAddress("localhost", 8081);
	...
	public static void readAndSend(DatagramChannel receiveChannel, Iterator<SelectionKey> iterator) throws IOException {
		while (iterator.hasNext()) {
			...
			if (key.isReadable()) {
				...
				buf2.clear();
				buf2.put("服务端反馈 the sky is mine 世界".getBytes());
				buf2.flip();
				// 往服务端发送，这个通道是服务端绑定的
				receiveChannel.send(buf2, new InetSocketAddress("localhost", 8081));
